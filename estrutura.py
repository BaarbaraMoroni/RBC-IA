...
def janelaInicial():
  ...
  coluna1 = [
      ...
      [
          sg.Combo([
              'Baixas Áreas', 'Espalhado', 'Áreas Superiores', 'Campo Inteiro'
          ],
                   key='areaDamaged',
                   enable_events=True,
                   readonly=True)
      ...
      [
          sg.Combo(['Sem Resposta', 'Marrom', 'dk-marrom-blk', 'Bronzeado'],
                   key='canker-lesion',
                   enable_events=True,
                   readonly=True)
      ...
      [
          sg.Combo([
              'Diferente do primeiro ano', 'Mesmo do primeiro ano',
              'Mesmo do ultimo ano', 'Mesmo do ultimo dois anos'
          ],
                   key='crop-hist',
                   enable_events=True,
                   readonly=True)
      ...
      [
          sg.Combo([
              'Não tem resposta', 'Ausente', 'Marrom com manchas pretas',
              'Colorido'
          ],
                   key='manchasdeFrutas',
                   enable_events=True,
      ...
      [
          sg.Combo(['Ausente', 'Presente'],
                   key='corposdeFrutificacao',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Vagens de Frutas')],
      [
          sg.Combo(['Não tem resposta', 'pouco presente', 'Normal', 'Doente'],
                   key='vagensdeFrutas',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Germinação')],
      [
          sg.Combo(['abaixo de 80%', '80-89 %', '90-100%'],
                   key='germinacao',
                   enable_events=True,
                   readonly=True)
      ]
  ...
  coluna2 = [
      ...
      [
          sg.Combo(['Ausente', 'Baixo surf', 'Alto surf'],
                   key='folhaSuave',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Folhagem')],
      ...
      [
          sg.Combo(['Ausente', 'Sem halo amarelo', 'halo amarelo'],
                   key='manchasHalo',
                   enable_events=True,
                   readonly=True)
      ...
      [
          sg.Combo(
              ['Nenhuma das alternativas', 'Abaixo de 1/8', 'Acima de 1/8'],
              key='tamanhodaMancha',
              enable_events=True,
              readonly=True)
      ], [sg.Text('Manchas de Folhas-Marg')],
      [
          sg.Combo(['Nenhuma das alternativas', 'no-w-s-marg', 'w-s-marg'],
                   key='manchasdeFolhasMarg',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Folhas')],
      [
          sg.Combo(['Anormal', 'Normal'],
                   key='folhas',
                   enable_events=True,
                   readonly=True)
      ]
  ]
  coluna3 = [[sg.Text('Alojamento')],
             [
                 sg.Combo(['Não', 'Sim'],
                          key='alojamento',
                          enable_events=True,
             ...
             [
                 sg.Combo(['Anormal', 'Normal'],
                          key='crescimentodaPlanta',
                          enable_events=True,
                          readonly=True)
             ], [sg.Text('Estande de Plantas')],
             [
                 sg.Combo(['Abaixo do normal', 'Normal'],
                          key='estandedePlantas',
                          enable_events=True,
                          readonly=True)
             ], [sg.Text('Precipício')],
             [
                 sg.Combo(['Abaixo do normal', 'Normal', 'Acima do Normal'],
                          key='precipício',
                          enable_events=True,
                          readonly=True)
             ], [sg.Text('Raízes')],
             [
                 sg.Combo(['Cistos de Vesícula', 'Normal', 'Apodrecido'],
                          key='raiz',
                          enable_events=True,
                          readonly=True)
             ], [sg.Text('Sclerotia')],
  ...
  coluna4 = [
      ...
      [
          sg.Combo(['Ausente ', 'Presente'],
                   key='descoloracaodaSemente',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Tamanho da Semente')],
      [
          sg.Combo(['Abaixo do normal', 'Normal'],
                   key='tamanhodaSemente',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Semente-Tmt')],
      [
          sg.Combo(['Nenhum', 'Fungicida', 'Outros'],
                   key='sementeTmt',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Gravidade')],
      [
          sg.Combo(['Menor', 'Grave', 'Severo'],
                   key='gravidade',
                   enable_events=True,
                   readonly=True)
      ], [sg.Text('Murchando')],
...
def janelaSecundaria(lista, lista2, lista3, cursor):
  ...
  coluna1 = [[
      sg.Table(values=listaP,
               select_mode=sg.SELECT_MODE_BROWSE,
               headings=['  Caso  ', 'Problema'],
  ...
  for casos in lista2:  # ListaDoencas
    cursor.execute("SELECT DescDoenca FROM bancoprincipal WHERE Caso = ?",
                   (casos[0], ))
  ...
  coluna3 = [[
      sg.Table(values=listaPP,
               select_mode=sg.SELECT_MODE_BROWSE,
               headings=[' Caso ', 'Doenca', 'Porcentagem'],
               auto_size_columns=False,
               expand_x=True,
               key='-TB2-',
               enable_events=True)
  ]]
  layout = [[sg.Column(coluna1), sg.VerticalSeparator(), sg.Column(coluna3)]]
...
def janelaFinal(listaP, casoSel, cursor):
  ...
  layout = [[sg.Text('Solução:' + resultado[0][2])],
            [sg.Column(coluna1),
